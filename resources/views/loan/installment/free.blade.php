		...
		<form method="post" action="{{URL::to('/installment/free_store', $angsuran->id)}}" enctype="multipart/form-data">
			...
			<div class="modal-body">
				...
				<?php
					...
					$last = App\Models\Installment::where('loan_number',$loanNumber)->orderBy('id','desc')->first();
					...
					$sisaBayaran = App\Models\Installment::where('loan_number',$loanNumber)->where('reminder', '>', 0)->first();
					$sisa = !empty($sisaBayaran->reminder) ? $sisaBayaran->reminder : 0;
					$kontrak = App\Models\CustomerContract::where('customer_id',$loans->customer_id)->first();
					...
					{						
						...
						$totalBayar = $byrCicilan + $byrTempo + !empty($tabungan) ? $tabungan : 0;
					}					
				?>

...
<div id="Update-{{$angsuran->id}}" class="modal fade in"  aria-labelledby="myModalLabel" style="display: none;" aria-hidden="true" role="dialog">
				...
				<div class="modal-body">
					<div class="form-group col-md-12">
						<label for="name" class="control-label">{{ trans('installment.customer_name') }}</label>												
							<input class="form-control" name="trans_number" type="text" value="{{$angsuran->trans_number}}" id="customer_name" disabled>	
					</div>
					<div class="form-group col-md-12 {!! $errors->has('tr_date') ? 'has-error' : '' !!} required ">
						<label for="name" class="control-label">{{ trans('installment.date') }}</label>													
							<div class="input-group">
								...
								<input class="form-control" name="pay_date" type="date" value="{{$angsuran->pay_date}}" id="pay_date" required>
							</div>
						...
						<label for="name" class="control-label">Status</label>						
							<select name="status" class="form-control" required>
								...
								@if ($angsuran->status === 'UNPAID')							
									<option value="UNPAID" selected="true">UNPAID</option>
									<option value="PAID">PAID</option>								
									<option value="PARTIAL">PARTIAL</option>
									<option value="CORRUPT">CORRUPT</option>							
								@elseif($angsuran->status === 'PAID')	
									<option value="UNPAID">UNPAID</option>
									<option value="PAID" selected="true">PAID</option>								
									<option value="PARTIAL">PARTIAL</option>
									<option value="CORRUPT">CORRUPT</option>
								@elseif($angsuran->status === 'PARTIAL')	
									<option value="UNPAID">UNPAID</option>
									<option value="PAID">PAID</option>								
									<option value="PARTIAL" selected="true">PARTIAL</option>
									<option value="CORRUPT">CORRUPT</option>
								@elseif($angsuran->status === 'CORRUPT')	
									<option value="UNPAID">UNPAID</option>
									<option value="PAID">PAID</option>								
									<option value="PARTIAL">PARTIAL</option>
									<option value="CORRUPT" selected="true">CORRUPT</option>
								@endif
							</select>						
					</div>
					<div class="form-group col-md-12 {!! $errors->has('amount') ? 'has-error' : '' !!} required ">
					...
					<div class="form-group col-md-12 {!! $errors->has('amount') ? 'has-error' : '' !!} required ">
						<label for="name" class="control-label">{{ trans('installment.amount') }}</label>												
							<input class="form-control" name="amount" type="text" value="{{$angsuran->amount}}" id="inputku" onkeydown="return numbersonly(this, event);" onkeyup="javascript:tandaPemisahTitik(this);" required>

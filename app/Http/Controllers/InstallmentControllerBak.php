...
class InstallmentController extends Controller
{
	...
	public function table_create($loan_number)
	{
		...
		{
			$getCustId = !empty($customer->id) ? $customer->id : NULL;
		}
				...
				if($jumlah > 1) {
					...
					elseif($amount >= $loans->loan_remaining)
                    {
						...
						try {
							...
							$checkSisa = Installment::where('loan_number', '=', $request->loan_number)->where('id', '<', $id)->orderBy('id','desc')->
								where('status','PAID')->
								first();
						...
						// done ga bang ? done. alhamdulillah
                    }else if ($amountRequest >= $loans->pay_month/2 + $totalTempo){
						try{
							$loan = Loan::where('loan_number', $request->loan_number)->first();
...
//                    untuk melunasi sisa berdasarkan amount request. dan jika amount request kurang dari pembayaran bulanan.
					...
					try{
						...
						$installB->t_installment = str_replace('.', '', $request->amount);
	...
	public function print($id)
    {
		...
		if (empty($installment))
		{
            ...
            $tempos = Tempo::where('member_number', $loans->member_number)->
			where('status', '=', 'PAID')->
			where('is_paid',true)->
			where('inst_to','=',$installmentTemps->inst_to)->first();
					...
					foreach($cicilans as $key => $row )
					{
						...
						$response .= "<tr>";
							...
							if ($sisa->sisa < 0) {
								...
								}else if ($findTempo->total_amount > 0 && $findTempo->inst_to == $row->inst_to){
